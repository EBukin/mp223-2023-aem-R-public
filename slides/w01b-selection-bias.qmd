...
-   We use "|" to denote **conditional** on $D_i$;
...
& = \underbrace{[Y_{i=1}|D_{i=1}=1] - [Y_{0,i=1}|D_{i=1}=1]}_\text{Average treatment effect on treated} \\
...
$$
\begin{align}
\text{Diff. between two HH} & = \underbrace{2 - (-2)}_\text{4} + \underbrace{(-2) - 1}_\text{-3} = \underbrace{1}_\text{Biased effect of a treatment}
\end{align}
$$
...
& = \underbrace{\color{Blue}{E[Y_{i}|D_{i}=1]} - \color{Green}{E[Y_{0i}|D_{i}=1]}}_\text{Average causal effect} \\
...
$$
= \underbrace{E[Y_{0i} + \rho|\color{Blue}{D_{i}=1}]}_{E[Y_{1i}|\color{Blue}{D_{i}=1}]} - E[Y_{0i}|\color{Red}{D_{i}=0}] 
$$
...
With random assignment, $\text{Diff. in group means} = \text{ATE}$ works because of the Law of Large Numbers (LLN)!
...
::: columns
::: {.column width="50%"}
Video 1: [Selection Bias]( https://youtu.be/6YrIDhaUQOE)
{{< video https://youtu.be/6YrIDhaUQOE >}}
:::

::: {.column width="50%"}
Video 2: [Randomized Trials](https://youtu.be/eGRd8jBdNYg)
{{< video https://youtu.be/eGRd8jBdNYg >}}
:::
:::

### Read (Optional): 

[@Angrist2014, chapter 1; @Angrist2009, chapter 2]
...
2.  Selection bias of means comparison;

3.  Actual and potential outcomes framework;

4.  Counterfactual;

5.  Role of research dsign in fighting with the selection bias.
